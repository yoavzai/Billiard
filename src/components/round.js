...
import { endRound,
        ...
        isParticipantAvailable} from "./utils"
...
export default function RoundComp() {
    ...
    const [isFinishRoundErrorMessage, setIsFinishRoundErrorMessage] = useState(false)
    const [isFinishRoundConfirmation, setIsFinishRoundConfirmation] = useState(false)
    ...
    function sortedRoundParticipants() {
        let sortedParticipants = participants.sort((a,b) => {
            const player1 = currentRound.data.arrivedParticipants.find(p => p.participantId === a.id)
            const player2 = currentRound.data.arrivedParticipants.find(p => p.participantId === b.id)
            if (player1 === undefined && player2 === undefined) {
                ...
                if (player1.isFree && player2.isFree) {
                    ...
                    const player1TableNumber = tables.find(t => t.data.participant1Id === player1.participantId || t.data.participant2Id === player1.participantId).data.number
                    const player2TableNumber = tables.find(t => t.data.participant1Id === player2.participantId || t.data.participant2Id === player2.participantId).data.number
    ...
    function getParticipantTableNumber(participantId) {
        const table = tables.find(t => (t.data.participant1Id === participantId || t.data.participant2Id === participantId) && t.data.isTaken)
        ...
        {Object.keys(currentRound).length > 0 &&
            <div className="container round_container">
                <div className="buttons_container">
                    <button className="button close_button" onClick={() => dispatch({type: "currentRound", payload: {}})}>סגור</button>
                </div>
                <h2>
                    <span>סיבוב</span>
                    <span>{" " + currentRound.data.number + " - "}</span>
                    <span>{" " + currentRound.data.startDate.day + " " + currentRound.data.startDate.month + " " + currentRound.data.startDate.year + " "}</span>
                    <span>{currentRound.data.isActive ? "(פעיל)" : "(הסתיים)"}</span>
                </h2>
                <div>
                    {currentRound?.data?.isActive &&
                    <div className="buttons_container">
                        <button className="button" onClick={finishRoundBtnClick}>סיים סיבוב</button>
                    </div>
                    }
                    ...
                    <div className="error_message_container">
                        ...
                        <div className="buttons_container">
                            <button className="button ok_button" onClick={() => setIsFinishRoundErrorMessage(false)}>Ok</button>
                        </div>
                    </div>
                    }
                    ...
                    <div className="confirmation_container">
                        ...
                        <div className="buttons_container">
                            <button className="button yes_button" onClick={finishRoundConfirmed}>כן</button>
                            <button className="button no_button" onClick={() => setIsFinishRoundConfirmation(false)}>לא</button>
                        </div>
                    </div>
                    }
                        ...
                        <table className="table">
                            ...
                            return (
                                <tr key={participant.id}>
                                    <td>
                                        <RoundParticipant participant={participant}
                                            name={getPlayerByParticipantIdFromStore(participant.id, participants, players).data.name}
                                    ...
                                    {currentRound.data.isActive && isParticipantAvailable(currentRound, participant.id) &&
                                    <td>
                                        <ul>
                                            {getOptionalParticipantsIdsToPlay(currentRound, participant.id, participants, players)
