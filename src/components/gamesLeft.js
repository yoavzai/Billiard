...
import { gamesLeftAboveAvarge, getPlayerByParticipantIdFromStore } from "./utils"
...
export default function GamesLeftComp(props) {
    ...
    return (
        <div className="games_left_container">
            <span className={`${gamesLeftAboveAvarge(participants, participant) ? "red" : ""}`}>
                {participant?.data?.participantsToPlayIds.length}
            </span>
            <div className="buttons_container">
                <button className="button info_button" onClick={() => setIsParticipantsToPlay(!isParticipantsToPlay)}>מידע</button>
            </div>
            {isParticipantsToPlay &&
            <div className="participants_to_play_list_container">
                <ul className="list">
                ...
                .map(participantId => {
                    return (
                        <li key={participantId}>{getPlayerByParticipantIdFromStore(participantId, participants, players)?.data?.name}</li>
                    )
                })}
                </ul>
            </div>}
