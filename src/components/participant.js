...
export default function ParticipantComp(props) {
    ...
    const [isRemoveParticipantErrorMessage, setIsRemoveParticipantErrorMessage] = useState(false)
    const [removeParticipantErrorMessage, setRemoveParticipantErrorMessage] = useState("")
    const [isUpdatePlayerErrorMessage, setIsUpdatePlayerErrorMessage] = useState(false)
    const [updatePlayerErrorMessage, setUpdatePlayerErrorMessage] = useState("")
    const [isRemoveParticipantConfirmation, setIsRemoveParticipantConfirmation] = useState(false)
    

    useEffect(() =>{
        setPlayer(getPlayerByParticipantIdFromStore(participant.id, participants, players))
    }, [players])
    ...
    function removeParticipantConfirmed() {
        ...
        deleteParticipant(currentTournament.id,
                            participant.id,
                            currentRound,
                            participants,
                            tables,
    ...
    async function removeParticipantBtnClick() {
        ...
        for (const table of tables) {
            if((table.data.participant1Id === participant.id || table.data.participant2Id === participant.id) && table.data.isTaken) {
    ...
    return (
        <div className="active_participant_container">
            ...
            {currentTournament.data.isActive &&
                isRemoveParticipantConfirmation ?
                <div className="confirmation_container">
                    <span>מחיקת משתתף מהטורניר תמחק את כל התוצאות שלו. האם אתה בטוח?</span>
                    <div className="buttons_container">
                        <button className="button yes_button" onClick={removeParticipantConfirmed}>כן</button>
                        <button className="button no_button" onClick={() => setIsRemoveParticipantConfirmation(false)}>לא</button>
                    </div>
                </div> :
                ...
                <div className="error_message_container">
                    ...
                    <div className="buttons_container">
                        <button className="button ok_button" onClick={() => setIsRemoveParticipantErrorMessage(false)}>Ok</button>
                    </div>
                </div> :
                isUpdatePlayer ?
                <div className="update_player_form_container">
                    <form>
                        <div className="text_input_container">
                            ...
                            <input className={`${isUpdatePlayerErrorMessage ? "error_input" : ""}`}
                                name="name" type="text" defaultValue={player.data.name}
                                onChange={(e) => {setPlayerNewData({...playerNewData, name: e.target.value}); setIsUpdatePlayerErrorMessage(false)}}></input>
                        </div>
                        <div className="select_container">
                            <label htmlFor="ranking-select">דירוג</label>
                            <select defaultValue={player.data.ranking} name="ranking-select"
                                    onChange={(e) => setPlayerNewData({...playerNewData, ranking: e.target.value})}>
                        ...
                        <div className="buttons_container">
                            <input disabled={playerNewData.name.length === 0 ? true : false} type="button" onClick={updatePlayerData} value="עדכן"></input>
                            <input type="button" onClick={cancelEditPlayer} value="בטל"></input>
                        </div>
                    </form>
                ...
                <div>
                    <div className="buttons_container">
                        <button className="button edit_button" onClick={editPlayer}>ערוך</button>
                        <button className="button delete_button" onClick={removeParticipantBtnClick}>מחק</button>
                    </div>
                </div>
            }
