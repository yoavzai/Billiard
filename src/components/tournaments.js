...
export default function TournamentsComp() {
    ...
    const [isNewTournamentConfirmation, setIsNewTournamentConfirmation] = useState(false)
    const [startDate, setStartDate] = useState({fullDate: "", month: "", year: ""})
    const [isStartTournamentErrorMessage, setIsStartTournamentErrorMessage] = useState(false)
    const [tournamentErrorMessage, setTournamentErrorMessage] = useState("")

    useEffect(() => {
        dispatch({type: "tournamentSelected", payload: {currentTournament: {},
                                                        participants: [], 
                                                        rounds: [],
                                                        currentRound: {},
                                                        standings: [],
    ...
    async function startNewTournament() {
        const tourId = await addTournament({startDate: startDate, isActive: true}, dispatch)
    ...
    function newTournamentBtnClick() {
        for (const tour of tournaments) {
            ...
            if (tour.data.isActive) {
                setIsNewTournamentConfirmation(false)
                setIsStartTournamentErrorMessage(true)
                setTournamentErrorMessage("אי אפשר להתחיל טורניר חדש בזמן שקיים טורניר פעיל")
                return
            }
    ...
    return (
        <div className="tournaments_container">
                    ...
                    {tournaments.map(tour => {
                        return (
                            <option
                                    key={tour.id}
                                    value={tour.id}
                                    >{`${tour?.data?.startDate.month} ${tour?.data?.startDate.year} ${tour?.data.isActive ? "(פעיל)" : "(הסתיים)"}`}
                            </option>
                        )
                    })}
                </select>
            ...
            <div className="buttons_container">
                <button className="button" onClick={newTournamentBtnClick}>התחל טורניר חדש</button>
            </div>
            ...
            <div className="confirmation_container">
                ...
                <span>?</span>
                <div className="buttons_container">
                    <button className="button yes_button" onClick={startNewTournament}>כן</button>
                    <button className="button no_button" onClick={() => setIsNewTournamentConfirmation(false)}>לא</button>
                </div>                      
            </div>
            }
            ...
            <div className="error_message_container">
                ...
                <div className="buttons_container">
                    <button className="button ok_button" onClick={() => setIsStartTournamentErrorMessage(false)}>Ok</button>
                </div>
            </div>
            }
        </div>
