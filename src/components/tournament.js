...
import { endTournament, erasePlayoffData, getPlayoff16, getPlayoff8, loadTournamentData, updatePlayoff16, updatePlayoff8 } from "./utils";
...
export default function TournamentComp() {
  ...
  async function startPlayoff8() {
    ...
    updatePlayoff8(currentTournament, playoff8, {"first": {}, "second": {}, "third": {}}, dispatch)
    setIsPlayoff16(false)
    setIsPlayoff8(true)
  }
  ...
  async function startPlayoff16() {
    ...
    updatePlayoff16(currentTournament, playoff16, {"first": {}, "second": {}, "third": {}}, dispatch)
    setIsPlayoff8(false)
    setIsPlayoff16(true)
  }
    ...
    <div>
      {Object.keys(currentTournament).length === 0 ? (
        ...
        <div className="tournament_container">
          ...
          {currentTournament.data.playoff8.length === 0 && currentTournament.data.playoff16.length === 0
          ?
          <div className="buttons_container">
            <button className="button" onClick={startPlayoff8}>התחל פלייאוף 8</button>
            <button className="button" onClick={startPlayoff16}>התחל פלייאוף 16</button>
          </div>
          :
          currentTournament.data.playoff8.length > 0
          ?
          <div className="buttons_container">
            <button className="button" onClick={openPlayoff8}>פלייאוף 8</button>
          </div>
          :
          <div className="buttons_container">
            <button className="button" onClick={openPlayoff16}>פלייאוף 16</button>
          </div>
          }
          ...
          <div>
            ...
            <div>
              <button onClick={() => setIsPlayoffErrorMessage(false)}>OK</button>
            </div>
          </div>
          }
