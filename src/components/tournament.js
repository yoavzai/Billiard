...
import { endTournament,
         loadTournamentData } from "./utils"
...
export default function TournamentComp() {
    ...
    const [isFinishTournamentErrorMessage, setIsFinishTournamentErrorMessage] = useState(false)
    const [isFinishTournamentConfirmation, setIsFinishTournamentConfirmation] = useState(false)
    ...
    return (
        <div>
            ...
            <div  className="tournament_container">
                ...
                <h2>
                    <span>טורניר</span>
                    <span>{" " + currentTournament?.data?.startDate.month + " " + currentTournament?.data?.startDate.year}</span>
                    <span>{currentTournament?.data?.isActive ? " (פעיל)" : " (הסתיים)"}</span>
                </h2>
                {currentTournament?.data?.isActive &&
                    <div className="buttons_container">
                        <button className="button" onClick={finishTournamentBtnClick}>סיים טורניר</button>
                    </div>
                }
                ...
                <div className="error_message_container">
                    ...
                    <div className="buttons_container">
                        <button className="button ok_button" onClick={() => setIsFinishTournamentErrorMessage(false)}>Ok</button>
                    </div>
                </div>
                }
                ...
                <div className="confirmation_container">
                    ...
                    <div className="buttons_contianer">
                        <button className="button yes_button" onClick={finishTournamentConfirmed}>כן</button>
                        <button className="button no_button" onClick={() => setIsFinishTournamentConfirmation(false)}>לא</button>
                    </div>
                </div>
                }
